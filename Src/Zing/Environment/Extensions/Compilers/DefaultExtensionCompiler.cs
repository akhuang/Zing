    ...
    public class DefaultExtensionCompiler : IExtensionCompiler
    {
        ...
        public void Compile(CompileExtensionContext context)
        {
            ...
            try
            {
                ...
                foreach (var assemblyReference in projectFileDescriptor.References)
                {
                    ...
                    else
                    {
                        ...
                        throw new ZingCoreException(T("Assembly reference '{0}' for project '{1}' cannot be loaded", assemblyReference.FullName, context.VirtualPath));
                    }
                }
            }
            catch (Exception e)
            {
                ...
                throw new ZingCoreException(T("Error compiling module \"{0}\" from file \"{1}\":\r\n{2}", moduleName, context.VirtualPath, e.Message), e);
            }
        }
